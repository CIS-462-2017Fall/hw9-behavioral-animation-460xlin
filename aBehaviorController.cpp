...

{
	...
	{ 
		...
		gOriKp = 256;
		gOriKv = 32;
		//m_force[0] = gMass * gVelKv * (m_vd - m_state[2][0]);
		m_force[0] = 0.0f;
		m_force[1] = 0.0f;
		m_force[2] = gMass * gVelKv * (m_vd - m_state[2][2]);

		double deltaAngle = (m_thetad - m_Euler[1]);
		ClampAngle(deltaAngle);
		m_torque[0] = 0.0;
		m_torque[1] = gInertia * (-gOriKv * m_stateDot[1][1] + gOriKp * deltaAngle);		
		m_torque[2] = 0.0;
...
//  This function sets derive vector to appropriate values after being called
{
	...
	//vec3 v0 = vec3(cos(theta), 0, -(sin(theta)));
	//vec3 v1 = vec3(0, 1, 0);
	//vec3 v2 = vec3(sin(theta), 0, cos(theta));

	vec3 v0 = vec3(cos(theta), 0, sin(theta));
	vec3 v1 = vec3(0, 1, 0);
	vec3 v2 = vec3(-sin(theta), 0, cos(theta));

	mat3 temp_0 = mat3(v2, v1, v0);

	m_stateDot[0] = temp_0 * m_state[2];
	m_stateDot[1] = m_state[3];
	m_stateDot[2] = m_force / gMass;
